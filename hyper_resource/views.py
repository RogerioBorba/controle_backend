...
CORS_ALLOW_HEADERS = ('accept', 'accept-encoding', 'authorization','content-type','content-location', 'dnt','origin','user-agent', 'x-csrftoken','x-requested-with', 'link')
CORS_ALLOW_EXPOSES = ('accept', 'accept-encoding', 'authorization','content-type','content-location', 'dnt','origin','user-agent', 'x-csrftoken','x-requested-with', 'link', 'access-control-allow-origin', 'access-control-allow-methods', 'access-control-allow-headers','access-control-max-age',)
...
ACCESS_CONTROL_ALLOW_METHODS = ['GET', 'OPTIONS', 'HEAD', 'PUT', 'POST', 'DELETE', 'PATCH']
...
class AbstractResource(APIView):
    ...
    def access_control_expose_headers_str(self):
        expose_headers_str = ''
        for header_value in CORS_ALLOW_EXPOSES:
    ...
    def add_methods_cors_header_in_header(self, response, method_names):
        response['access-control-allow-origin'] = self.access_control_allow_origin_str()
        response['access-control-allow-headers'] = self.access_control_allow_headers_str()
        response['access-control-allow-methods'] = ','.join(method_names)
    ...
    def set_key_with_data_in_cache(self, key, etag, data, seconds=0):
        """
    ...
    def head(self, request, *args, **kwargs):
        ...
        self.add_base_headers(request, resp)
        self.add_cors_header_in_header(resp)
        return resp
    ...
    def delete(self, request, *args, **kwargs):
        ...
        obj.delete()
        resp = Response(status=status.HTTP_204_NO_CONTENT)
        self.add_methods_cors_header_in_header(resp, ['DELETE'])
        return resp
...
class FeatureCollectionResource(SpatialCollectionResource):
    ...
    def basic_response(self, request, objects):
        ...
        cache.set(iri_with_content_type,(local_hash, serialized_data), 0)
        return resp
